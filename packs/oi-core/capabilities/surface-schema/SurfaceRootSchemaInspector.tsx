    ...
    <div class="mt-2 text-sm space-y-4">
      {Object.entries(jsonMap).map(([targetField, rule]) => (
        ...
        >
          ...
          {rule.Description && (
            <div class="text-neutral-400">{rule.Description}</div>
          )}
        </div>
      ))}
    </div>
...
export function SurfaceRootSchemaInspector({
  ...
  onDetailsChanged: _,
  onToggleEnabled,
}: SurfaceRootSchemaInspectorProps) {
        ...
        tabs={[
          {
            ...
            label: 'Schema',
            content: (
              <SchemaDefinitionTab details={details as EaCRootSchemaDetails} />
            ),
          },
          {
            key: 'mapping',
            label: 'Field Mapping',
            content: (
              <FieldMappingTab details={details as EaCRootSchemaDetails} />
            ),
          },
          {
            key: 'analytics',
